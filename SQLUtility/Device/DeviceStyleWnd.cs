...
{
    public partial class DeviceStyleWnd : Form
    {
        public DeviceStyleWnd()
        {
            InitializeComponent();
        ...
        private void LoadList()
        {
            ...
            string sql = string.Format("SELECT * FROM DeviceStyle");
            try
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                // 查询用的sql语句
                string sql = string.Format("SELECT COUNT(*) FROM DeviceStyle WHERE 设备类型='{0}'",
            ...
            {
                //构造insert语句
                string sql = "INSERT INTO DeviceStyle (设备类型) VALUES(@设备类型)";
                //构造sql语句的参数
                MySqlParameter[] ps = //使用数组初始化器
                {
                new MySqlParameter("@设备类型",cboDeviceProducer.Text),
                };
                //执行插入操作
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (row != null)
            {
                result = MessageBox.Show("确实要删除该类型吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes) // 确认删除
                {
                    string sql = string.Format("DELETE FROM DeviceStyle WHERE 设备类型='{0}'",
