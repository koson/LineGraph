...
{
    public partial class Goods_Management : Form
    {
        string sql = "";       //查询条件
         ...
         try
            {
                ...
                sql = "select goods_ID as 序号,SN as 商品编号,goods_Name as 商品名称, goods_Property as 商品属性, goods_Company as 商品生产厂商 from goods2";
            ...
            try
            {
                if (e.ColumnIndex == this.dataGridView1.Columns["删除"].Index)
                {
                    ...
                    if (id != null && id != "" && MessageBox.Show("您确定要删除吗？", "重要提示！", MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation) == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        conn.Open();
                        sql = "delete from Sensor2 where goods_ID=" + id;
